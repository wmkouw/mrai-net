...
for n in range(lT):
	for i in range(nI):
		...
		with open('results_manual/err_exp_acqinv_cnn_b1b3_manual_nT'+str(nT[n])+'_r'+str(iI[i]+1)+'.pickle', 'rb') as handle:
...
# Create figures
fig2, axs = plt.subplots(nrows=1, ncols=2, sharex=True, figsize=(20,6))
fig2.suptitle('Brainweb1.5T $->$ Brainweb3.0T',fontsize=fS, fontweight='heavy')

# Scanner error total
ax0 = axs[0]
ax0.plot(nT,scan_m[0,:], 'r', label='raw')
...
ax2.plot(nT,tiss_m[1,:], 'm', label='s+t')
...
# Write results
fig2.savefig('vis/err-lc_exp_acqinv_cnn_b1b3_manual_H.png', bbox_inches='tight', pad_inches=0)
