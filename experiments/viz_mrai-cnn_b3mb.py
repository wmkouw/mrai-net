...
for n in range(lT):
	for i in range(nI):
		...
		with open('results_cnn/err_exp_acqinv_cnn_b3mb_nT'+str(nT[n])+'_r'+str(iI[i]+1)+'.pickle', 'rb') as handle:
...
fig2.suptitle('Brainweb3.0T $->$ MRBrains',fontsize=fS, fontweight='heavy')
...
# Write results
fig2.savefig('vis/err-lc_exp_acqinv_cnn_b3mb_H.png', bbox_inches='tight', pad_inches=0)
