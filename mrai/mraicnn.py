...
import keras.regularizers as krg
import sklearn.feature_extraction.image as sfe
...
class MRAIConvolutionalNeuralNetwork(object):
    ...
    def compile_net(self):
        ...
        # Setup input layers
        A = kly.Input(shape=(*self.patch_size, 1))
        B = kly.Input(shape=(*self.patch_size, 1))
        sIDA = kly.Input(shape=(1,))
        sIDB = kly.Input(shape=(1,))

        # Convolutional pipeline
        pipeC = kmd.Sequential()
        pipeC.add(kly.Conv2D(self.num_kernels[0],
                             self.kernel_size[0],
                             activation='relu',
                             padding='valid',
                             kernel_regularizer=krg.l2(self.l2),
                             input_shape=(*self.patch_size, 1)))
        pipeC.add(kly.MaxPooling2D(pool_size=(2, 2), padding='valid'))
        pipeC.add(kly.Dropout(self.dropout))
        for n in range(1, len(self.num_kernels)):
            pipeC.add(kly.Conv2D(self.num_kernels[n],
                                 self.kernel_size[n],
                                 activation='relu',
                                 padding='valid',
                                 kernel_regularizer=krg.l2(self.l2)))
            pipeC.add(kly.MaxPooling2D(pool_size=(2, 2), padding='valid'))
            pipeC.add(kly.Dropout(self.dropout))
        pipeC.add(kly.Flatten())

        # kly.Dense pipeline
        pipeD = kmd.Sequential()
        pipeD.add(kly.Dense(self.dense_size[0],
                            activation='relu',
                            kernel_regularizer=krg.l2(self.l2),
                            input_shape=(pipeC.output_shape[1] + 1,)))
        for n in range(1, len(self.dense_size)):
            pipeD.add(kly.Dropout(self.dropout))
            pipeD.add(kly.Dense(self.dense_size[n],
                                activation='relu',
                                kernel_regularizer=krg.l2(self.l2)))
        pipeD.add(kly.Dense(2))

        # Distance in embedding space
        distance = kly.Lambda(self.l1_norm, output_shape=(1,))(
                          [pipeD(kly.concatenate([pipeC(A), sIDA])),
                           pipeD(kly.concatenate([pipeC(B), sIDB]))])

        # Set model
        model = kmd.Model(inputs=[A, B, sIDA, sIDB], outputs=distance)
    ...
    def contrastive_loss(self, label, distance):
        ...
        """
        return kbK.sum(label * kbK.square(distance) +
                       (1-label) * kbK.maximum(self.margin - distance, 0))

    def l1_norm(self, x):
        """l1-norm for loss layer."""
        return kbK.sum(kbK.abs(x[0] - x[1]), axis=1, keepdims=True)

    def l2_norm(self, x):
        """l2-norm for loss layer."""
        return kbK.sum(kbK.square(x[0] - x[1]), axis=1, keepdims=True)
    ...
    def extract_all_patches(self, X, scan_ID=0, edge=(0, 0)):
        ...
        patches = sfe.extract_patches_2d(X, patch_size=self.patch_size)
    ...
    def load_model(self, model_fn, weights_fn):
        ...
        self.net = kmd.model_from_json(loaded_model_json)
    ...
    def feedforward(self, patches, scan_ID):
        ...
        interm_model = kmd.Model(inputs=self.net.input,
